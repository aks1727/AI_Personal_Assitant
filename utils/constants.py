class ImmutableMeta(type):
    ...
    def __setattr__(cls, name, value):
        raise AttributeError(
            f"Immutable Error: Cannot reassign final constant '{name}' on class '{cls.__name__}'."
        )

    def __delattr__(cls, name):
        raise AttributeError(
            f"Immutable Error: Cannot delete final constant '{name}' on class '{cls.__name__}'."
        )
...
class AudioConstants(metaclass=ImmutableMeta):
    ...
    STABLE_PLAYER_COMMAND = (
        f"ffplay -nodisp -autoexit -loglevel quiet {TEMP_AUDIO_MP3} 2> /dev/null"
    )
